...
function Get-DefaultGlobalGitIgnoreTemplate {
  ...
  $TemplatePath = Join-Path -Path $ProfileDir -ChildPath "Template\GlobalGitIgnoreTemplate.txt"

  if (Test-Path $TemplatePath) {
    $msg = "♻️ Global git ignore template found ♻️"

    Write-Host -NoNewline (Get-CenteredPadding -RawMessage $msg)

    # Read file and clean up line breaks
    return Get-Content -Path $TemplatePath -Encoding UTF8 | ForEach-Object { $_.TrimEnd("`r") }
  }
  else {
    $msg = "❌ Global git ignore template not found ❌"

    Write-Host -NoNewline (Get-CenteredPadding -RawMessage $msg)
    Write-Host ""

    Write-Host -NoNewline "Target path : " -ForegroundColor Cyan
    Write-Host " $TemplatePath" -ForegroundColor DarkCyan
